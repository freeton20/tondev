...
function loadBinaryVersions(name) {
    return __awaiter(this, void 0, void 0, function* () {
        const info = yield httpsGetJson(`https://binaries.tonlabs.io/${name}.json`);
...
function installGlobally(dstPath, version, terminal) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield run("npm", ["install", "-g"], { cwd: binDir }, terminal);
        }
        catch (err) {
...
function downloadFromGithub(terminal, srcUrl, dstPath) {
    return __awaiter(this, void 0, void 0, function* () {
        terminal.write(`Downloading from ${srcUrl}`);
        if (!fs.existsSync(dstPath)) {
            fs.mkdirSync(dstPath, { recursive: true });
        }
        yield downloadAndUnzip(dstPath, srcUrl, terminal);
        terminal.write("\n");
    });
}
...
function downloadFromBinaries(terminal, dstPath, src, options) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (srcExt === ".zip") {
                yield downloadAndUnzip(dstDir, srcUrl, terminal);
            }
            else if (srcExt === ".gz") {
                yield downloadAndGunzip(dstPath, srcUrl, terminal);
                if (path.extname(dstPath) === ".tar") {
                    yield run("tar", ["xvf", dstPath], { cwd: path.dirname(dstPath) }, terminal);
                    fs.unlink(dstPath, () => { });
                }
            }
            else {
            ...
            if (options === null || options === void 0 ? void 0 : options.globally) {
                ...
                yield installGlobally(dstPath, options.version, terminal).catch(err => {
                    fs.unlink(dstPath, () => { });
                    throw err;
                });
            }
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const spawned = isWindows
                ? (0, child_process_1.spawn)("cmd.exe", ["/c", name].concat(args), Object.assign({ env: process.env }, options))
                : (0, child_process_1.spawn)(name, args, Object.assign({ env: process.env }, options));
            const output = [];
            ...
            spawned.on("close", code => {
                if (code === 0) {
                    ...
                    reject(Error(terminal instanceof StringTerminal
                        ? terminal.stderr
                        : `${name} failed`));
                }
            });
...
function formatTable(table, options) {
    ...
    const formatRowLine = (rowLine) => rowLine
        .map(formatValue)
...
function resolveTvcAsBase64(filePath) {
...
exports.resolveTvcAsBase64 = resolveTvcAsBase64;
function resolveContract(filePath) {
    var _a;
    filePath = filePath.trim();
    const lowered = filePath.toLowerCase();
    let basePath;
    if (lowered.endsWith(".tvc") ||
...
function readTextFileSyncOnce(filename) {
    try {
...
exports.readTextFileSyncOnce = readTextFileSyncOnce;
function getLatestFromNmp(pkgName) {
    return __awaiter(this, void 0, void 0, function* () {
        const latestVer = yield run("npm", ["view", pkgName, "dist-tags.latest"], {}, new StringTerminal());
        return latestVer.trim();
    });
}
exports.getLatestFromNmp = getLatestFromNmp;
